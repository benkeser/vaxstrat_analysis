...
cfg <- yaml::read_yaml("config_provide.yml")
config <- cfg[[setting]]
...
results <- future.apply::future_lapply(1:nrow(grid), function(i, grid){
  ...
  results_df <- cbind(results_df, data.frame(# Naturally infected AIPW
    ...
    nat_inf_upper_bound_reject = results$nat_inf$bound$reject$additive,
    # Doomed AIPW
    ...
    doomed_upper_bound_reject = results$doomed$bound$reject$additive,
    # Pop AIPW
